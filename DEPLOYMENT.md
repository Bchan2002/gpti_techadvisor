...
6. Espera a que termine el deployment (puede tardar 5-10 min)
   - El script `start.sh` ejecutará automáticamente el seed para crear el admin y los computadores
...
El script `start.sh` ya creó automáticamente:
...
5. **Auto-seed**: El script `start.sh` ejecuta el seed automáticamente cada vez que se reinicia el backend
...
### Backend no inicia después del deploy
- Revisa los logs en Render
- Verifica que todas las variables de entorno estén configuradas
- El script `start.sh` debe tener permisos de ejecución (ya configurado)
